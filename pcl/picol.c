...
int     picolEval2(picolInterp *i, char *t, int mode) { /*----------- EVAL! */
  ...
  while(1) {
    ...
    if(i->trace)
    {
	char *ptr = strstr(t, "assert");
	if(ptr)
	{
	    ptr = strstr(p.start, "assert");
...
COMMAND(info) {
  ...
  /*ARITY2*/"info args|body|commands|exists|globals|level|procs|vars")
 ...
 exists, globals, level, procs, vars", argv[1]);
  return PICOL_OK;
}
COMMAND(interp) {
...
#else
COMMAND(trace) {
  ARITY2(argc==2, "trace add|remove");
  if     (EQ(argv[1],"add"))    i->trace = 1;
  else if(EQ(argv[1],"remove")) i->trace = 0;
  else return picolErr1(i,"Bad option %s, must be add, or remove",argv[1]);
  return PICOL_OK;
